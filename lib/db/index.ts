...
import { assertDbHostAllowed } from './guard.mjs'
...
const url = process.env.VE_DATABASE_URL || process.env.DATABASE_URL!
// Cinturón de seguridad: nunca prod desde un entorno no-prod (ver lib/db/guard.mjs).
assertDbHostAllowed(url, 'runtime')
const sql = neon(url)
